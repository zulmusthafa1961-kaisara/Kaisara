...
void OnTick() {
   ...
   // Read all trend box on M5 from current candle to last bars 
   // search for previous box. If the previous box is "UP" then stay out
   // if the previous box is "DOWN" then we can look for Buy trading opportunity on the current box which is green
   
   
   
   }
   else if(trendstrH1=="DOWN"){
